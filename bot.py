...
boss = ['Jonnlena']#Usuarios administradores 
...
save_cred = {"humberto.rodriguezl":{"ID":None,"TOKEN":"Ndtdcm382b5Mr3g"}}
control_upload = {}
bytes_control = {}
TEMP_FILE = {}
...
async def start(client: Client, message: Message):
	...
	if comprobacion_de_user(username) == False:
		...
		await bot.send_message(chat_id='28146160', text=intento_msg)
...
async def cloud(client: Client, message: Message):
	...
	await send_config()
	await send("✅ 𝑫𝒐𝒏𝒆")
#comando para activar subida por nextcloud manualmente
...
async def nube(client: Client, message: Message):
	...
	Configs[username]["z"] = 99
	await send_config()
	await send("☁️ Nube Nextcloud Activada ☁️")
#Encriptar con xdlink los txt
...
#NO usar este comando pueds romper el bot dejarlo como esta de fabrica
...
#comando para saber la configuracion y espacio ne la nube nextcloud
...
async def splase(user, passw, host, proxy,username):
  ...
  async with aiohttp.ClientSession(connector=proxy) as session:
   ...
   #comando para configurar cuentas manualmente     	
...
async def config(client: Client, message: Message):	
	...
	await send("💬 Configuracion establecida 💬")
...
async def zips(client: Client, message: Message):
	...
	await send_config()
	await send("🗳 Tamaño de archivo establecido 🗳")
...
async def zips(client: Client, message: Message):
	...
	await send_config()
	await send("🚀 Proxy establecido con exito 🚀")
...
async def delete_my(client: Client, message: Message):
	...
	if Configs[username]["a"] == "a":
		...
		if u == False:
			await msgcheck.edit(f"⚠️ 𝑶𝒄𝒖𝒓𝒓𝒊𝒐 𝒖𝒏 𝑬𝒓𝒓𝒐𝒓 𝒐 𝒏𝒐 𝒉𝒂𝒚 𝒆𝒍𝒆𝒎𝒆𝒏𝒕𝒐𝒔 𝒑𝒂𝒓𝒂 𝒃𝒐𝒓𝒓𝒂𝒓 ⚠️")
			return
		else:
...
async def delete_nex(client: Client, message: Message):
	...
	if all_delete:
		...
		await f.edit("♻️ 𝐓𝐨𝐝𝐨𝐬 𝐥𝐨𝐬 𝐚𝐫𝐜𝐡𝐢𝐯𝐨𝐬 𝐡𝐚𝐧 𝐬𝐢𝐝𝐨 𝐛𝐨𝐫𝐫𝐚𝐝𝐨𝐬 ♻️")
	else:
		await f.edit("⚠️ Ocurrió un error, inténtelo denuevo, revise sus credenciales y/o directorio ⚠️")
...
async def seven(client: Client, message: Message):
	...
	if len(lista) == 2:
		...
		if not "." in j:
			...
			await h.edit("✂️ 𝑫𝒊𝒗𝒊𝒅𝒊𝒆𝒏𝒅𝒐 𝒆𝒏 𝒑𝒂𝒓𝒕𝒆𝒔 ✂️")
...
async def callback_data(bot,callback):
	...
	if "uo" in callback.data:
	  if not username in TEMP_FILE:
	    await callback.message.edit("⚠️ No hay Archivos para Subir ⚠️")
	    return
	...
	if "delete" in callback.data:
		try:
			...
			#print(data)
			await callback.message.delete()
			msgs = await bot.send_message(username,"♻️ Eliminando ♻️")
...
async def up(client: Client, message: Message):	
	...
	if username not in boss and Configs["s"] == "Off":
		await client.send_message(username,'⚠️ Modo mantenimiento activado no puede usar este comando ⚠️')
		return
	else: pass	
	print(14)
	comp = comprobar_solo_un_proceso(username) 
...
async def cancel(client: Client, message: Message):	
	...
	if id_de_ms[username]["proc"] == "Up":
		...
		await client.send_message(username,"⚠️ 𝑵𝒐 𝒉𝒂𝒚 𝒑𝒓𝒐𝒄𝒆𝒔𝒐𝒔 𝒅𝒆 𝒔𝒖𝒃𝒊𝒅𝒂 𝒒𝒖𝒆 𝒄𝒂𝒏𝒄𝒆𝒍𝒂𝒓 ⚠️")
		return
...
async def supr_process(client: Client, message: Message):	
	...
	if username in boss:
		procesos = 0
		await send(f"✅ Procesos borrados con exito ✅")
	else:return

@bot.on_message(filters.command("mantenimiento", prefixes="/") & filters.private)
async def change_status(client: Client, message: Message):	
	...
	if username in boss:
		...
		await send("⚠️ 𝑪𝒐𝒎𝒂𝒏𝒅𝒐 𝒑𝒂𝒓𝒂 𝒂𝒅𝒎𝒊𝒏𝒊𝒔𝒕𝒓𝒂𝒅𝒐𝒓𝒆𝒔 ⚠️")
		return
...
def update_progress_bar(inte,max):
	...
	progress_bar = progress_bar[:percentage_pos] + percentage_string + progress_bar[percentage_pos]
	return(progress_bar)
...
def download_progres(data,message,format):
	if data["status"] == "downloading":
		...
		_format_str = format		
		msg = f"📦 𝐍ombre: {filename}\n\n"
		msg+= f"▶️ 𝙳escargando: {_downloaded_bytes_str} of {_total_bytes_str}\n\n"
...
async def downloadmessage_progres(chunk,filesize,filename,start,message):
		...
		msg+= f"▶️ 𝙳escargando: {sizeof_fmt(chunk)} of {sizeof_fmt(filesize)} \n\n"	
		global seg
		if seg != localtime().tm_sec:
...
def uploadfile_progres(chunk,filesize,start,filename,message):
	...
	msg+= f"▶️ Subiendo: {sizeof_fmt(chunk)} of {sizeof_fmt(filesize)}\n\n"
	global seg
	if seg != localtime().tm_sec:
		message.edit(msg)
	seg = localtime().tm_sec
async def mediafiredownload(chunk,total,filename,start,message):
	...
	msg+= f"▶️ 𝙳escargando: {sizeof_fmt(chunk)} of {sizeof_fmt(total)}\n\n"
	global seg
	if seg != localtime().tm_sec:
		try: await message.edit(msg,reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Cancel","cancel_button")]]))
		except MessageNotModified:
...
def monitor_callback(monitor, msg, filesize, filename, start,uploaded_size):
	...
	diff = now - start
	mbs = chunk / diff
	ms = f"📦 𝐍ombre: {filename}\n\n"
	try:
		ms+=update_progress_bar(chunk,filesize)+ "  " + sizeof_fmt(mbs)+"/s\n\n"
	except:pass
	ms+= f"▶️ Subiendo: {sizeof_fmt(uploaded_size)} of {sizeof_fmt(filesize)}\n\n"
	global seg
	if seg != localtime().tm_sec:
		msg.edit(ms)
	seg = localtime().tm_sec
...
def proccess(filex,user_id,msg,username):
    ...
    #			asyncio.run( msg.delete())
                for file in files:
                    ...
                    try:
                        msg = asyncio.run( msg.edit(f'🚀 Subiendo ...\n\n{filepath}'))
                    except:
                        pass
